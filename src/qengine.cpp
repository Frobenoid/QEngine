...
void QEngine::draw() {
  ...
  qutils::transition_image(cmd, _drawImage.image, VK_IMAGE_LAYOUT_UNDEFINED,
  ...
  qutils::copy_image_to_image(cmd, _drawImage.image,
...
void QEngine::init_swapchain() {
  ...
  VkImageCreateInfo rimg_info = qinit::image_create_info(
  ...
  VkImageViewCreateInfo rview_info = qinit::imageview_create_info(
...
void QEngine::init_commands() {
  ...
  VkCommandBufferAllocateInfo cmdAllocInfo =
      qinit::command_buffer_allocate_info(_immCommandPool, 1);
...
void QEngine::draw_background(VkCommandBuffer cmd) {
...
void QEngine::init_background_pipelines() {
  ...
  if (!qutils::load_shader_module("../shaders/compute.comp.spv", _device,
                                  &computeDrawShader)) {
...
void QEngine::immediate_submit(
  ...
  VkCommandBufferSubmitInfo cmdinfo = qinit::command_buffer_submit_info(cmd);
  VkSubmitInfo2 submit = qinit::submit_info(&cmdinfo, nullptr, nullptr);

  VK_CHECK(vkQueueSubmit2(_graphicsQueue, 1, &submit, _immFence));
  VK_CHECK(vkWaitForFences(_device, 1, &_immFence, true, 99999999));
}

void QEngine::init_imgui() {
